...
void checkSolenoids() {
  if (onSolenoidP && now - TonSolenoidP > SOLPTIME && digitalRead(ttl1) == LOW && digitalRead(but1) == HIGH) {
    onSolenoidP = false;
    digitalWrite(sol1, false);
  }
  else if (onSolenoidM && now - TonSolenoidM > SOLMTIME && digitalRead(ttl3) == LOW && digitalRead(but3) == HIGH) {
    onSolenoidM = false;
    digitalWrite(sol3, false);
  }
  else if (onSolenoidN && now - TonSolenoidN > SOLNTIME && digitalRead(ttl2) == LOW && digitalRead(but2) == HIGH) {
    onSolenoidN = false;
    digitalWrite(sol2, false);
  }
  
  else if (!onSolenoidP && !onSolenoidM && !onSolenoidN && (digitalRead(ttl1) == HIGH || digitalRead(but1) == LOW)) {
    onSolenoidP = true;
    digitalWrite(sol1, true);
    TonSolenoidP = now;
  }
  else if (!onSolenoidP && !onSolenoidM && !onSolenoidN && (digitalRead(ttl3) == HIGH || digitalRead(but3) == LOW)) {
    onSolenoidM = true;
    digitalWrite(sol3, true);
    TonSolenoidM = now;
  }
  else if (!onSolenoidP && !onSolenoidM && !onSolenoidN && (digitalRead(ttl2) == HIGH || digitalRead(but2) == LOW)) {
    onSolenoidN = true;
